...
//  the rest of the elements to shift and occupy the freed place. We expect to have a shorter array now.
...
// The syntax is:
// ‌‍‌⁡⁢⁢⁢// 𝘢𝘳𝘳.𝘴𝘱𝘭𝘪𝘤𝘦(𝘴𝘵𝘢𝘳𝘵[, 𝘥𝘦𝘭𝘦𝘵𝘦𝘊𝘰𝘶𝘯𝘵, 𝘦𝘭𝘦𝘮1, ..., 𝘦𝘭𝘦𝘮𝘕])
...
// If an argument argN is an array,
//  then all its elements are copied. Otherwise, the argument itself is copied.
...
//  if we look for false, it finds exactly false and not the zero.

// If we want to check if item exists in the array and don’t need the index, then arr.includes is preferred.

// The method arr.lastIndexOf is the same as indexOf, but looks for from right to left.

let fruits = ['Aplle', 'Orange', 'Apple']
...
let lengths = ar.map(item => item.length);
...
// The arr can be an array of anything, right?
...
//  then reduce takes the first element of the array as the initial value and starts the iteration from the 2nd element.
...
// then army.canJoin would be called as a standalone function, with this=undefined,
...
//  if fn returns a falsy value, arr.every() immediately returns false and stops iterating over the rest of items
//  as well.
...
// This copies elements from one part of the array and pastes them somewhere else in the SAME array.

// Think:

// Copy + Paste inside the same array
// Syntax
// arr.copyWithin(target, start, end)
// target → where to paste
// start → where to copy from
// end → stop copying (not included)
// Example 1
