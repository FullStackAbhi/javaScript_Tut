...
//  as we mentioned earlier in the chapter Data types. 
...
1e6 = 1000000
1e-6 = 0.000001 //1/1000000
// Just like before, using "e" can help. If we’d like to avoid writing the zeroes explicitly,
...
let num = 122382736872346723

alert( num.toString(36) ); // 2n9c
alert( 122382736872346723..toString(36) ); // 2n9c

// Two dots to call a method
// Please note that two dots in 123456..toString(36) is not a typo. 
// If we want to call a method directly on a number,
...
alert( 0.1 + 0.2 ); // 0.30000000000000004 == 0.3 => false
// Ouch! Imagine you’re making an e-shopping site and the visitor puts $0.10 and $0.20 goods into their cart.
...
//  But fractions like 0.1, 0.2 that look simple in the decimal numeric system are actually unending fractions
...
//  such numbers are easily representable. Compare it to one-third: 1/3. It becomes an endless fraction 0.33333(3).

// So, division by powers 10 is guaranteed to work well in the decimal system, but division by 3 is not
// . For the same reason, in the binary numeral system, the division by powers of 2 is guaranteed to work,
//  but 1/10 becomes an endless binary fraction.
...
// This suffers from the same issue: a loss of precision. There are 64 bits for the number,
//  52 of them can be used to store digits, but that’s not enough. 
// So the least significant digits disappear.

// JavaScript doesn’t trigger an error in such events. It does its best to fit the number into the desired format, 
...
// But in real life, we often have values in units, like "100px" "11rem" 
// or "12pt" in CSS. 
// Also in many countries, the currency symbol goes after the amount, so we have "19€" and would like to
...
alert( parseFloat('12.34') ); // 12.3, the second point stops the reading
