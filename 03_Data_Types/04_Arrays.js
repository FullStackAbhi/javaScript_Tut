...
// However, in JavaScript it won’t work. The result will be undefined, because the index in square brackets is treated 
// literally.
...
//  advancing the queue, so that the 2nd element becomes the 1st.
...
// actually come from the object syntax. That’s essentially the same as obj[key],
...
//  They provide special methods for that. Arrays are carefully tuned inside JavaScript engines to work with
//  contiguous ordered data, please use them this way. And if you need arbitrary keys, 
...
//  but they may also have other non-numeric properties and methods
// , which we usually don’t need. The for..in loop will list them though.
//  So if we need to work with array-like objects, then these “extra” properties can become a problem.

// The for..in loop is optimized for generic objects, not arrays, 
// and thus is 10-100 times slower. Of course, it’s still very fast. 
...
// These arrays are technically different objects.
//  So they aren’t equal. The == operator doesn’t do item-by-item comparison.
...
// That’s simple: don’t use the == operator. Instead,
//  compare them item-by-item in a loop or using iteration methods explained in the next chapter.
...
// The length property is the array length or, to be precise, 
// its last numeric index plus one. It is auto-adjusted by array methods.
...
//  For negative values of i, it steps back from the end of the array. If i >= 0, it works same as arr[i].
